...
if __name__ == '__main__':
    ...
    bm25_retriever.k = 4  # 4 跳记录

    # 3.创建FAISS向量数据库检索
    faiss_db = FAISS.from_documents(documents, embedding=OpenAIEmbeddings(
    ...
    # 4.初始化集成检索器
    ensemble_retriever = EnsembleRetriever(  # 记录进行合并
        retrievers=[bm25_retriever, faiss_retriever],
        weights=[0.5, 0.5],  # 合并权重一样
    )
